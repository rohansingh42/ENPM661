...
class node:
    def __init__(self, x, y, theta, parentid, g, d = 0):
        self.x = np.round(x, 0)
        self.y = np.round(y, 0)
        self.theta = np.round(theta, 0)
        self.id = str(self.x) + str(self.y) + str(self.theta)
...
class PathPlan:
    ...
    RPM1 = 100             # LOW RPM (rev/min)
    RPM2 = 500             # HIGH RPM (rev/min)
    wr = 38               # Wheel Radius (mm)
    wb = 240              # Wheel Base (mm)
    f = 1                # Sampling Frequency (1/sec)
    rr = 177              # Robot Radius (mm)
    c = 0                 # Clearance (mm)
    gt = 177              # Radius of circle that is considered goal position (mm)
    motion_model = [[0,RPM1],                
              [RPM1,0],     
              [RPM1,RPM1],               
              [0,RPM2],    
    ...
    def nextNode(self,cnode,step):
        ...
        # dx = (self.wr/2) * (ul + ur) * math.cos(theta*(math.pi/180)/2) * (1/self.f)
        # dy = (self.wr/2) * (ul + ur) * math.sin(theta*(math.pi/180)/2) * (1/self.f)
    ...
    def gen_path(self,closed):
        ...
        while parentid != self.start.id:
            ...
            print(closed[parentid])
            parentid = closed[parentid].parentid
            xdata.append(closed[parentid].x)
            ydata.append(closed[parentid].y)
    ...
    def checkCollision(self,cnode):
        ...
        r14_3 = x - (-4550.4) < 0
        r14_4 = x - (-1952.7) > 0
        c14_1 = (x + 4550.4)**2 + (y - 3250.5)**2 - (299.5 + r)**2 < 0
        c14_2 = (x + 1952.7)**2 + (y - 3250.5)**2 - (299.5 + r)**2 < 0
